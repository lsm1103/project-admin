...
func genEtc(dir string, cfg *config.Config, api *spec.ApiSpec) error {
	...
	// ==========add==========
	dataSource := api.Info.Properties["dataSource"]
	cacheHost := api.Info.Properties["cacheHost"]
	if host_,ok := api.Info.Properties["host"]; ok{
		host = host_
	}
	if port_,ok := api.Info.Properties["port"]; ok{
		port = port_
	}
	// ==========add==========
