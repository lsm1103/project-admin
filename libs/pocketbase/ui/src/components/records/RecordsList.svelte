...
{#if records.length}
    <small class="block txt-hint txt-right m-t-sm">显示 {records.length} of {totalRecords}</small>
{/if}
...
{#if records.length && canLoadMore}
    <div class="block txt-center m-t-xs">
        ...
        >
            <span class="txt">加载更多 ({totalRecords - records.length})</span>
        </button>
    </div>
{/if}
...
{#if totalBulkSelected}
    <div class="bulkbar" transition:fly|local={{ duration: 150, y: 5 }}>
        <div class="txt">
            选中了 <strong>{totalBulkSelected}</strong>
            条记录
        </div>
        <button
            type="button"
