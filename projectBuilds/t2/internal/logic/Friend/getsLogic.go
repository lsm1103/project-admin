...
func (l *GetsLogic) Gets(req *sqlUtils.GetsReq) (resp *types.FriendList, err error) {
	// 自动生成的后台管理接口  req *types.GetsReq
	resp = &types.FriendList{Current: req.Current, PageSize: req.PageSize}
	err = l.svcCtx.FriendModel.FindAll(req, &resp.List)
	if err != nil {
		return nil, err
	}
