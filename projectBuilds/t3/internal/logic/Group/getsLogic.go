...
func (l *GetsLogic) Gets(req *sqlUtils.GetsReq) (resp *types.GroupList, err error) {
	// 自动生成的后台管理接口
	resp = &types.GroupList{Current: req.Current, PageSize: req.PageSize}
	err = l.svcCtx.GroupModel.FindAll(req, &resp.List)
	if err != nil {
		return nil, err
	}
