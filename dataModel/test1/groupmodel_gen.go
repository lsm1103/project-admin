...
var (
	...
	groupRowsExpectAutoSet   = strings.Join(stringx.Remove(groupFieldNames, "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`state`"), ",")
	groupRowsWithPlaceHolder = strings.Join(stringx.Remove(groupFieldNames, "`id`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`"), "=?,") + "=?"
...
func (m *defaultGroupModel) Insert(ctx context.Context, session sqlx.Session, data *Group) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, data.Id, data.Name, data.CreateUser, data.Ico, data.Remark, data.ParentId, data.GroupType, data.Rank, data.State)
		}
		return conn.ExecCtx(ctx, query, data.Id, data.Name, data.CreateUser, data.Ico, data.Remark, data.ParentId, data.GroupType, data.Rank, data.State)
	}, groupCreateUserNameKey, groupIdKey)
	return ret, err
}

func (m *defaultGroupModel) Update(ctx context.Context, session sqlx.Session, newData *Group) error {
	data := &Group{}
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, newData.Name, newData.CreateUser, newData.Ico, newData.Remark, newData.ParentId, newData.GroupType, newData.Rank, newData.State, newData.Id)
		}
