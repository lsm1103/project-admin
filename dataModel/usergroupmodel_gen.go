...
var (
	...
	userGroupRowsExpectAutoSet   = strings.Join(stringx.Remove(userGroupFieldNames, "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`state`"), ",")
	userGroupRowsWithPlaceHolder = strings.Join(stringx.Remove(userGroupFieldNames, "`id`", "`update_time`", "`create_at`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`"), "=?,") + "=?"
...
func (m *defaultUserGroupModel) Insert(ctx context.Context, session sqlx.Session, data *UserGroup) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, data.Id, data.UserId, data.GroupId, data.State)
		}
		return conn.ExecCtx(ctx, query, data.Id, data.UserId, data.GroupId, data.State)
	}, userGroupIdKey, userGroupUserIdGroupIdKey)
	return ret, err
}

func (m *defaultUserGroupModel) Update(ctx context.Context, session sqlx.Session, newData *UserGroup) error {
	data := &UserGroup{}
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, newData.UserId, newData.GroupId, newData.State, newData.Id)
		}
