...
func Test_BuildDataModel(t *testing.T) {
	t.Log("")
...
func Test_BuildApiFile(t *testing.T)  {
	//生成api文件
	dataModelToApi.DataModelToApi{
		...
		dataModelToApi.ServiceInfo{
			...
			Filename: fmt.Sprintf("%s/deploy/init.sql", RootPkgPath),
			Database: "",
			Strict: false,
		},
	}.StartBuild()
	t.Log("dataModelToApi")
}

func Test_BuildApiService(t *testing.T) {
	err := BuildCode(BuildCode{
		RootPkgPath: RootPkgPath,
		Info:        BuildAppInfo{
			Title:   "项目11",
	...
	if err != nil {
		t.Log( errors.Wrapf(xerr.NewErrCode(xerr.USER_OPERATION_ERR),
			"生成api服务代码失败：%s", err.Error()))
	}
}

func Test_BuildSwaggerDoc(t *testing.T) {
	err := BuildCode(BuildCode{
		RootPkgPath: RootPkgPath,
		Info:        BuildAppInfo{
			Title:   "项目11",
