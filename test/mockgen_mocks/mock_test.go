...
func TestMySQL_CreateSingleMsg(t *testing.T) {
	ctr := gomock.NewController(t)
	defer ctr.Finish()
	mock := NewMockTest2Mock(ctr)
	gomock.InOrder(
		mock.EXPECT().CreateSingleMsg(types.CreateSingleMsg{
			Id:               0,
			SenderType:       "",
			SenderId:         0,
			SenderDeviceId:   "",
	...
	err := mock.CreateSingleMsg(types.CreateSingleMsg{
		Id:               0,
		SenderType:       "",
		SenderId:         0,
		SenderDeviceId:   "",
...
func TestMySQL_GetSingleMsg(t *testing.T) {
	ctr := gomock.NewController(t)
	defer ctr.Finish()
	mock := NewMockTest2Mock(ctr)
	gomock.InOrder(
		mock.EXPECT().GetSingleMsg(types.GetSingleMsg{
			Id: 1,
		}).Return(&types.SingleMsg{}, nil),
	)
	r, err := mock.GetSingleMsg(types.GetSingleMsg{
		Id: 1,
	})
	if err != nil {
		return
...
func TestMySQL_GetsSingleMsg(t *testing.T) {
	...
	mock := NewMockTest2Mock(ctr)
	gomock.InOrder(
		mock.EXPECT().GetsSingleMsg(types.GetsReq{
			Query:    nil,
			OrderBy:  "id",
			Sort:     "",
			Current:  1,
			PageSize: 20,
		}).Return(&types.SingleMsgList{
			List:     nil,
			Current:  1,
			PageSize: 20,
			IsNext:   false,
	...
	r, err := mock.GetsSingleMsg(types.GetsReq{
		Query:    nil,
		OrderBy:  "id",
		Sort:     "",
		Current:  1,
...
func TestMySQL_GetFullName(t *testing.T) {
	name := GetFullName()
	fmt.Println(name)
}